...
namespace BoulderLeaf::Graphics
{
	...
	std::unique_ptr<byte[]> CubeInline()
	{
		...
		std::unique_ptr<byte[]> cube(BuildInlineMesh<StandardVertex>(vertices.data(), vertices.size(), indices.data(), indices.size()));
	...
	namespace
	{
		...
		MeshDataPrototype CreateCylinderData(float bottomRadius, float topRadius, float height, unsigned int sliceCount, unsigned int stackCount)
		{
			...
			for (unsigned int i = 0; i < stackCount; ++i)
			{
				for (unsigned int j = 0; j < sliceCount; ++j)
				{
					meshData.indices.push_back((blMeshStorage::index)(i * ringVertexCount + j));
					meshData.indices.push_back((blMeshStorage::index)((i + 1) * ringVertexCount + j));
					meshData.indices.push_back((blMeshStorage::index)((i + 1) * ringVertexCount + j + 1));
					meshData.indices.push_back((blMeshStorage::index)(i * ringVertexCount + j));
					meshData.indices.push_back((blMeshStorage::index)((i + 1) * ringVertexCount + j + 1));
					meshData.indices.push_back((blMeshStorage::index)(i * ringVertexCount + j + 1));
				}
			}
